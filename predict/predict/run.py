...
class TextPredictionModel:
    def __init__(self, model, params, labels_index):
        self.model = model
        self.params = params
        self.labels_index = labels_index
        self.labels_index_inv = {ind: lab for lab, ind in self.labels_index.items()}
    ...
    def from_artefacts(cls, artefacts_path: str):
        ...
        return cls(model, params, labels_index)
    ...
    def predict(self, text_list, top_k=5):
        ...
        # predictions = argsort(predictions, axis=1)[:, -top_k:][:, ::-1]

        # TODO: CODE HERE
        # from tags indexes compute top_k tags for each text
        indices = argsort(predictions)[0][-top_k:]
        predictions = [self.labels_index.get(str(index)) for index in indices]
